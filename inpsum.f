...
90402      FORMAT(1X,'**Non-DFAULT option to ignore morning transition',
     ...
     &              ' URBAN SO2.'
         ...
         END IF
         IF (URBAN) THEN
            IF (POLLUT .EQ. 'NO2' .AND. ARM) THEN

             WRITE(IOUNT,*) '        6. Ambient Ratio ',
     &              'Method (ARM) Used for NO2 Conversion'
             WRITE(IOUNT,9993) ARM_1hr
9993         FORMAT('               with a  1-hour NO2/NOx Ratio ',
     &              'of ',F6.3)
             IF (ARM_Ann .GT. 0.0D0 .AND. ARM_Ann .LE. 1.0D0) THEN
                WRITE(IOUNT,9994) ARM_Ann
9994         FORMAT('               with an Annual NO2/NOx Ratio ',
     &               'of ',F6.3)
             END IF
...
C ---          User-specified HALFLIFE or DCAYCOEF
     ...
     &              ' Decay.'
         ...
         IF (PVMRM) THEN
            WRITE(IOUNT,'(9x,2a)') 
     &        '6. Plume Volume Molar Ratio Method (PVMRM)',
...
c   *******  End of ARM2
     ...
     &              'NO2.'
         ...
         IF (.NOT.PVMRM .AND. .NOT.OLM .AND. 
     &       .NOT.ARM .AND. .NOT.ARM2 .AND. POLLUT.NE.'NO2') THEN
            IF (URBAN) THEN
     ...
     &                  'of 1.0 Meter Used.'
               ...
               IF ( NUMCAP.GT.0 .OR. NUMHOR.GT.0 ) THEN
                  WRITE(IOUNT,*) '        7. Option for Capped &',
...
9092              FORMAT(12X,I6,' Capped Stack(s); and ',I6,
     &                          ' Horizontal Stack(s)')
            ...
            ELSE
               IF ( NUMCAP.GT.0 .OR. NUMHOR.GT.0 ) THEN
                  WRITE(IOUNT,*) '        6. Option for Capped &',
     &                            ' Horiz Stacks Selected With:'
                  WRITE(IOUNT,9099)
                  WRITE(IOUNT,9092) NUMCAP, NUMHOR
               END IF
     ...
     &                  'of 1.0 Meter Used.'
               ...
               IF (.NOT. L_UrbanTransition) THEN
                  IF ( NUMCAP.GT.0 .OR. NUMHOR.GT.0 ) THEN
                     WRITE(IOUNT,*) '        8. Option for Capped',
     &                              ' & Horiz Stacks Selected With:'
                     WRITE(IOUNT,9099)
                     WRITE(IOUNT,9092) NUMCAP, NUMHOR
...
C --- Look for other user-specified options to list in the MODEL SETUP
C     OPTIONS SUMMARY
      IF (NOWARN .OR. NOCHKD .OR. SCREEN .OR. FASTALL .OR. FASTAREA .OR.
     &                                         L_WARNCHKD .OR. SCIM .OR.
     &                   L_LowWind1 .OR. L_LowWind2 .OR. L_LowWind3 .OR.
     &                                   L_AdjUstar .OR. L_VECTORWS .OR.
     &                                    L_MMIF_Data .OR. L_BULKRN .OR.
     &                                  L_CCVR_Sub .OR. L_TEMP_Sub) THEN
         WRITE(IOUNT,9099)
     ...
     &                   'which forces calculation of centerline values'
      ...
      IF (L_LowWind1) THEN
         WRITE(IOUNT,*) '        LOWWIND1 - Use LowWind1 BETA option'
         IF (L_UserSVmin .OR. L_UserWSmin) THEN
         WRITE(IOUNT,*) '                   with user-specified ',
     &                                               'parameters'
         END IF

      ELSE IF (L_LowWind2) THEN
         WRITE(IOUNT,*) '        LOWWIND2 - Use LowWind2 BETA option'
     ...
     &                                               'parameters'
      ...
      ELSE IF (L_LowWind3) THEN
         WRITE(IOUNT,*) '        LOWWIND3 - Use LowWind3 BETA option'
         IF (L_UserSVmin .OR. L_UserWSmin .OR. L_UserFRANmax) THEN
         WRITE(IOUNT,*) '                   with user-specified ',
     &                                               'parameters'
         END IF
      ...
      IF (L_AdjUstar) THEN
         WRITE(IOUNT,*) '        ADJ_U*   - Use ADJ_U* BETA option ',
     &                                     'for SBL in AERMET'
      END IF

      IF (L_MMIF_Data) THEN
         WRITE(IOUNT,*) '        MMIFData - Use BETA option for ',
     &                                     'MMIF met data inputs'
     ...
     &                     'SolarRad option for SBL in AERMET'
      ...
      IF (L_VECTORWS) THEN
         WRITE(IOUNT,*) '        VECTORWS - User specifies that input ',
     &                                    'wind speeds are VECTOR means'
      END IF
...
C     Write Out Numbers of Sources, Groups, and Receptors for This Run
      WRITE(IOUNT,9099)
      IF (EVONLY) THEN
         WRITE(IOUNT,9046) NUMSRC, NUMGRP, NUMEVE, NUMPNT,NUMCAP,NUMHOR,
     &                     NUMVOL, NUMAREA, NUMLINE, NUMPIT, NUMBL, 
     &                     NBLINES
      ELSE IF (.NOT. EVONLY) THEN
         WRITE(IOUNT,9044) NUMSRC, NUMGRP, NUMREC, NUMPNT,NUMCAP,NUMHOR,
     &                     NUMVOL, NUMAREA, NUMLINE, NUMPIT, NUMBL, 
     &                     NBLINES
      END IF
...
C --- Summarize DEBUGOPTs selected by the user
     ...
     &    .or. OLMDEBUG .or. ARMDEBUG .or. ARM2DEBUG .or. DEPOSDBG )THEN
...
C ---    Loop through the 9 different options that are flagged
         DO I = 1, 9
...
C ---    Write message regarding MULTYEAR applications      
 ...
 9044 FORMAT(1X,'**This Run Includes: ',I6,' Source(s);  ',I6,
     ...
     &       16X,' and: ',I6,' BUOYANT LINE source(s) with ',
     &                    I6,' line(s)'/)
 ...
 9046 FORMAT(1X,'**This Run is for EVENT Processing.',
     ...
     &       16X,' and: ',I6,' BUOYANT LINE source(s) with ',
     &                    I6,' line(s)'/)
99048 FORMAT(21X,'BACKGRND Concentrations Varying by Direction Across',
     &       I6,' Sectors')
...
99014 FORMAT(2(3X,8(I2,2X,E9.4,3X)/),3X,8(I2,2X,E9.4,3X))
 9015 FORMAT(/26X,'* SECT',I1,' OZONE CONCENTRATIONS (',A5,') ',
...
C           BLOUTPROCESSED lets AERMOD know that all lines associated with
C            the buoyant line source were written on the first pass
...
C            as one source
     ...
     &              BLINEPARMS(NBL)%BLHS, URB, CQFLG
 9087	      FORMAT(1X,A12,2X,A5,2X,E11.5,4F10.1,A8,F9.2,7X,A3,3X,A7)
         END DO
         WRITE(IOUNIT,9088)
 9088	   FORMAT(/15X,'AVG BLDG',3X,'AVG BLDG',3X,'AVG BLDG',3X,
     ...
     &         /66(' -')/)
         WRITE(IOUNIT,9089) BLAVGBLEN, BLAVGBHGT, BLAVGBWID,
     &         BLAVGLWID, BLAVGBSEP, BLAVGFPRM
 9089	   FORMAT(13X,F8.1,3X,F8.1,3X,F8.1,3X,F8.1,6X,F8.1,10X,F8.1)
         BLOUTPROCESSED = .TRUE.
         END IF  
      END DO
...
C     Variable Declarations
      ...
      INTEGER :: ISECT, I, J, K, NL, I1, I2, I3, IFR, IDW, INDC, INGRP
      CHARACTER BLDING*3, URB*3, IQUN*12, CAP*3, CQFLG*7
      CHARACTER SEASON(4)*6, MONTHS(12)*9, DAYOFWEEK(3)*8,
     &          DAYOFWEEK7(7)*8, CNPD*5, CAZS*8
...
C     Print User-specified Background Concetrations
      ...
      ELSE IF (BFLAG(ISECT) .EQ. 'HRDOW') THEN
         ...
         DO I1 = 1, 3
            IDW = (I1-1)*24
            WRITE(IOUNIT,99021) DAYOFWEEK(I1)
            WRITE(IOUNIT,99014) (I3,BACKGRND(I3+IDW,1),I3=1,24)
         END DO
      ...
      ELSE IF (BFLAG(ISECT) .EQ. 'SHRDOW') THEN
         ...
         DO I1 = 1, 3
            ...
            DO I2 = 1, 4
               IFR = (I2-1)*24
               WRITE(IOUNIT,99019) SEASON(I2), DAYOFWEEK(I1)
               WRITE(IOUNIT,99014) (I3,BACKGRND(I3+IFR+IDW,1),I3=1,24)
            END DO
         END DO
...
99013 FORMAT(1X,65('- '))
 9014 FORMAT(4(5X,6(I3,3X,E10.5,4X)/))
99014 FORMAT(2(3X,8(I2,2X,E9.4,3X)/),3X,8(I2,2X,E9.4,3X))
 9015 FORMAT(/17X,'* SECT',I1,' BACKGROUND CONCENTRATIONS WHICH VARY ',
     &       'WITH WIND SPEED (',A5,') *'/)
